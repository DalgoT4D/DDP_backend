...
def get_public_chart_data_preview_total_rows(request, token: str, chart_id: int):
    ...
    try:
        ...
        logger.warning(f"Public total rows validation error for chart {chart_id}: {str(e)}")
...
def get_public_report_table_total_rows(request, token: str, chart_id: int):
    ...
    try:
        ...
        logger.warning(f"Public report total rows validation error for chart {chart_id}: {str(e)}")
        return 400, PublicErrorResponse(error=str(e), is_valid=False)
    except Exception as e:
