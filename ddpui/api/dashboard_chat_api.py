...
def _build_dashboard_system_prompt(
    ...
    prompt_parts = [
        ...
        "- When only schema/structure is available and no relevant values are present, NEVER invent or guess specific numbers. Clearly say that this metric is not available in the current dashboard data.",
        "- NEVER suggest that the user should write or run SQL or any other query; all querying is handled automatically by the platform.",
        "",
        ...
        "- Start answers to direct metric questions (e.g. 'How many deaths in Maharashtra in 2020?') with ONE short sentence giving the exact number from the data.",
        "- Optionally follow with ONE short sentence of context or interpretation if helpful.",
    ...
    if enhanced_analysis.get("tables_analyzed", 0) > 0:
        ...
        prompt_parts.extend(
            [
                ...
                "- Automatic calculation of totals and aggregations from detailed data",
                "- Understanding of data relationships and how to aggregate by dimensions",
            ]
        )
    ...
    if summary.get("data_included"):
        ...
        if charts_with_data > 0:
            prompt_parts.extend(
                [
                    ...
                    "- You have access to both aggregated chart results AND raw table data",
                    "- You can automatically execute additional queries if needed using existing intelligence layers",
            ...
            prompt_parts.extend(
                [
                    ...
                    "- This IS the final dashboard data - answer directly from chart results",
                    ...
                    "- For aggregated questions: Use the chart's aggregated values directly",
                    "- Example: 'Total for Africa: 55' (from aggregated chart data)",
                    ...
                    "- For grouped questions: Sum all rows for that group/dimension",
                    "- Example: 'Africa total: 162 + 391 + 100 = 653' (sum all matching rows)",
                    "",
                    "🚫 NEVER SAY:",
                    ...
                    "- 'the data available does not provide specific information' (when it actually does)",
                    "- 'charts and tables focus on different metrics' (analyze what's available)",
                    "",
                    "✅ ALWAYS SAY:",
                    ...
                    "- For grouped statistics: 'Total for Africa: 653' or 'Africa shows 55 in chart'",
                    "- Be decisive: 'Based on the dashboard data, Africa has X total'",
                    "",
                    "❓ IF DATA IS NOT AVAILABLE:",
                    "- Use ONLY the data provided in this context. If the required combination (for example a specific state and year) does not appear in the data rows or query results you see, clearly state that this specific metric is not available in the current dashboard data.",
                    "- Example: 'The current dashboard data does not contain deaths for Maharashtra in 2020, so I cannot give a number from this dashboard.'",
                    "- NEVER say 'you can query' or 'run a query' or 'I can help formulate' or anything that asks the user to write or execute queries.",
                    "",
                ]
            )
        else:
        ...
        prompt_parts.extend(
            [
                ...
                "You have access to real table schemas with actual column names and data types, but you do NOT have access to row-level values for calculations.",
                "State structural facts only: 'This table contains 8 columns including customer_name and revenue'.",
                "Be specific: 'The date column enables time-series analysis' not 'date columns might allow temporal analysis'.",
                "Make structural recommendations: 'Use customer_name for grouping and revenue for aggregation' not 'these columns could potentially be used'.",
                "For any question that requires an exact number (counts, totals, averages, percentages), clearly state that precise values cannot be computed because underlying data values are not shared in this mode.",
                "",
                "Note: User can enable data sharing in chat settings to analyze actual values and calculate precise metrics. Never guess or infer exact numbers when data sharing is disabled.",
                "",
            ]
        )
