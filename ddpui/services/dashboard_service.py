...
class DashboardService:
    ...
    @staticmethod
    def compute_field_changes(old_dashboard: Dashboard, new_dashboard: Dashboard) -> dict:
        """
        Compute field changes between old and new dashboard states for audit logging.

        Args:
            old_dashboard: Dashboard state before update
            new_dashboard: Dashboard state after update

        Returns:
            Dictionary of field changes for audit log
        """
        field_changes = {}

        # Track basic field changes
        if old_dashboard.title != new_dashboard.title:
            field_changes["title"] = {"old": old_dashboard.title, "new": new_dashboard.title}

        old_desc = old_dashboard.description or ""
        new_desc = new_dashboard.description or ""
        if old_desc != new_desc:
            field_changes["description"] = {"old": old_desc, "new": new_desc}

        # Track tab/layout changes
        old_tabs = old_dashboard.tabs or []
        new_tabs = new_dashboard.tabs or []

        if old_tabs != new_tabs:
            # Build tab lookup by id
            old_tabs_by_id = {tab.get("id"): tab for tab in old_tabs if tab.get("id")}
            new_tabs_by_id = {tab.get("id"): tab for tab in new_tabs if tab.get("id")}

            old_tab_ids = set(old_tabs_by_id.keys())
            new_tab_ids = set(new_tabs_by_id.keys())

            # Check for added/removed tabs
            for tab_id in new_tab_ids - old_tab_ids:
                tab_title = new_tabs_by_id[tab_id].get("title", tab_id)
                field_changes[f"Tab '{tab_title}'"] = {"old": None, "new": "added"}

            for tab_id in old_tab_ids - new_tab_ids:
                tab_title = old_tabs_by_id[tab_id].get("title", tab_id)
                field_changes[f"Tab '{tab_title}'"] = {"old": "removed", "new": None}

            # Check for changes in existing tabs
            for tab_id in old_tab_ids & new_tab_ids:
                old_tab = old_tabs_by_id[tab_id]
                new_tab = new_tabs_by_id[tab_id]
                tab_title = new_tab.get("title", tab_id)

                # Tab title changed
                if old_tab.get("title") != new_tab.get("title"):
                    field_changes["Tab renamed"] = {
                        "old": old_tab.get("title"),
                        "new": new_tab.get("title"),
                    }

                # Components changed - components is a dict
                old_components = old_tab.get("components", {})
                new_components = new_tab.get("components", {})

                # Handle both dict and list formats for backwards compatibility
                if isinstance(old_components, list):
                    old_components = {c.get("id", str(i)): c for i, c in enumerate(old_components)}
                if isinstance(new_components, list):
                    new_components = {c.get("id", str(i)): c for i, c in enumerate(new_components)}

                if old_components != new_components:
                    old_comp_ids = set(old_components.keys())
                    new_comp_ids = set(new_components.keys())

                    # Helper to get component description for audit log
                    def get_component_description(comp: dict) -> str:
                        comp_type = comp.get("type", "component")
                        config = comp.get("config", {})

                        # For chart components, use "Chart ID {chartId}"
                        if comp_type == DashboardComponentType.CHART.value:
                            chart_id = config.get("chartId")
                            if chart_id:
                                return f"Chart ID {chart_id}"

                        # For KPI components, use "KPI ID {kpiId}"
                        if comp_type == DashboardComponentType.KPI.value:
                            kpi_id = config.get("kpiId")
                            if kpi_id:
                                return f"KPI ID {kpi_id}"

                        # For other components (text, heading), use title or type
                        title = comp.get("title")
                        if title:
                            return f"{comp_type.capitalize()} '{title}'"
                        return comp_type.capitalize()

                    # Added components
                    for comp_id in new_comp_ids - old_comp_ids:
                        comp = new_components[comp_id]
                        comp_type = comp.get("type", "component").capitalize()
                        comp_desc = get_component_description(comp)
                        field_changes[f"{comp_type} added"] = {
                            "old": None,
                            "new": f"{comp_desc} added to tab '{tab_title}'",
                        }

                    # Removed components
                    for comp_id in old_comp_ids - new_comp_ids:
                        comp = old_components[comp_id]
                        comp_type = comp.get("type", "component").capitalize()
                        comp_desc = get_component_description(comp)
                        field_changes[f"{comp_type} removed"] = {
                            "old": f"{comp_desc} removed from tab '{tab_title}'",
                            "new": None,
                        }
