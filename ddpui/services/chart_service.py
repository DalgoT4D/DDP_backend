...
class ChartService:
    ...
    @staticmethod
    def compute_field_changes(old_chart: Chart, new_chart: Chart) -> dict:
        """
        Compute field changes between old and new chart states for audit logging.

        Args:
            old_chart: Chart state before update
            new_chart: Chart state after update
        ...
        if old_chart.title != new_chart.title:
            field_changes["title"] = {"old": old_chart.title, "new": new_chart.title}

        old_desc = old_chart.description or ""
        new_desc = new_chart.description or ""
        if old_desc != new_desc:
            field_changes["description"] = {"old": old_desc, "new": new_desc}

        if old_chart.chart_type != new_chart.chart_type:
            field_changes["chart_type"] = {
                "old": old_chart.chart_type,
                "new": new_chart.chart_type,
            }

        if old_chart.schema_name != new_chart.schema_name:
            field_changes["schema_name"] = {
                "old": old_chart.schema_name,
                "new": new_chart.schema_name,
            }

        if old_chart.table_name != new_chart.table_name:
            field_changes["table_name"] = {
                "old": old_chart.table_name,
                "new": new_chart.table_name,
            }

        # Track config changes
        old_config = old_chart.extra_config or {}
        new_config = new_chart.extra_config or {}

        if old_config != new_config:
            config_changes = ChartService._extract_config_changes(old_config, new_config)
            field_changes.update(config_changes)
    ...
    def _extract_config_changes(old_config: dict, new_config: dict, prefix: str = "config") -> dict:
        ...
        Args:
            ...
            new_config: New config dict
            prefix: Key prefix for nested changes

        Returns:
            Dictionary of config changes
        """
        changes = {}
            ...
            # Both are lists - compare
                ...
                if old_val != new_val:
                    if len(old_val) != len(new_val):
                        ...
                        changes[full_key] = {"old": "previous", "new": "modified"}
            # Simple values or type mismatch
            else:
