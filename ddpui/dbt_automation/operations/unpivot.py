...
def unpivot_dbt_sql(
    ...
    dbt_code += (
        "["
        + ",".join([quote_constvalue(col_name, warehouse.name) for col_name in exclude_columns])
        + "] ,"
    )
    ...
    dbt_code += (
        "["
        + ",".join([quote_constvalue(col_name, warehouse.name) for col_name in remove_columns])
        + "] ,"
    )
