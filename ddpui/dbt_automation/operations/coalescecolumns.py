...
def coalesce_columns_dbt_sql(
    ...
    elif isinstance(default_value, datetime.datetime) or isinstance(default_value, datetime.date):
    ...
    dbt_code += (
        "COALESCE("
        + ", ".join(
            [quote_columnname(col_name, warehouse.name) for col_name in coalesce_columns]
            + [default_value]
        )
