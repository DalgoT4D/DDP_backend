...
def test_get_table_count_no_warehouse(seed_db, orguser):
    """get_table_count returns 404 when no warehouse is configured"""
    request = mock_request(orguser)
    with pytest.raises(HttpError) as exc:
...
def test_get_table_count_success(seed_db, orguser):
    """get_table_count returns total_rows on success"""
    OrgWarehouse.objects.create(org=orguser.org, name="fake-warehouse-name")
...
def test_get_table_count_postgres_table_not_found(seed_db, orguser):
    """get_table_count returns 404 when Postgres table does not exist"""
    OrgWarehouse.objects.create(org=orguser.org, name="fake-warehouse-name")
...
def test_get_table_count_bigquery_table_not_found(seed_db, orguser):
    """get_table_count returns 404 when BigQuery table does not exist"""
    OrgWarehouse.objects.create(org=orguser.org, name="fake-warehouse-name")
...
def test_get_table_count_generic_error(seed_db, orguser):
    ...
    mock_client = Mock()
    mock_client.get_total_rows.side_effect = RuntimeError("connection refused")
