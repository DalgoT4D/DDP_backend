...
def test_same_credentials_reuse_one_engine():
    ...
    assert len(postgres_engine_registry._engines) == 1
...
def test_warehouses_differing_only_by_database_get_separate_engines():
    ...
    assert len(postgres_engine_registry._engines) == 2
...
def test_fingerprint_is_order_independent_but_type_scoped():
    ...
    same = postgres_engine_registry.fingerprint(
        "postgres", {"host": "h", "database": "d", "password": "p"}
    )
    reordered = postgres_engine_registry.fingerprint(
        "postgres", {"password": "p", "database": "d", "host": "h"}
    )
    other_wtype = postgres_engine_registry.fingerprint(
...
def test_pool_kwargs_bound_the_connection_ceiling():
    ...
    kwargs = postgres_engine_registry.pool_kwargs()

    assert kwargs["pool_size"] == postgres_engine_registry.POOL_SIZE
    assert kwargs["max_overflow"] == postgres_engine_registry.POOL_MAX_OVERFLOW
    assert kwargs["pool_timeout"] == postgres_engine_registry.POOL_TIMEOUT
...
def test_pre_ping_is_enabled():
    ...
    """
    assert postgres_engine_registry.pool_kwargs()["pool_pre_ping"] is True


def age_entry(key, seconds):
    """Backdate an entry's last-used stamp to simulate `seconds` of inactivity."""
    postgres_engine_registry._engines[key].last_used_at -= seconds
...
def test_registry_stats_reports_the_connection_ceiling():
    ...
    stats = postgres_engine_registry.registry_stats()
