...
# DB-backed test classes are marked individually so the pagination regression
# tests below can run without a test database (they only compile SQL).
...
# ---------------------------------------------------------------------------
# Pagination regression tests (no DB needed — they only compile SQL).
#
# Guard against the empty-SELECT bug where the column/aggregation/GROUP BY
# logic sat inside the `else: # No pagination` branch of build_chart_query.
# With pagination enabled the generated SQL was
# `SELECT FROM (SELECT * ... LIMIT n)` — no dimension, no metric, no GROUP BY —
# so every category rendered as the "Unknown" null label. The bug slipped
# through twice because nothing crashed: the query is valid SQL and the API
# returns 200.
# ---------------------------------------------------------------------------


def _pagination_org_warehouse(wtype="postgres"):
    ow = MagicMock()
    ow.wtype = wtype
    return ow


def _compile_sql(payload):
    qb = charts_service.build_chart_query(payload, _pagination_org_warehouse())
...
class TestPaginatedChartQueryKeepsColumns:
    ...
    def test_bar_chart_with_pagination_selects_dimension_metric_and_groups(self):
        sql = _compile_sql(_paginated_payload("bar"))
        sql_upper = sql.upper()
