...
get_or_create by natural keys makes it equivalent to running under
`apps.get_model`, and it lets the double-run (idempotency) and no-Redis
behavior be asserted inside the test transaction.
"""
...
seed_migration = import_module("ddpui.migrations.0171_seed_share_permission_slugs")

pytestmark = pytest.mark.django_db

NEW_SLUGS = {"can_share_reports", "can_share_alerts", "can_share_metrics", "can_share_kpis"}
...
# ================================================================================
# Existing-database world: the 0171 data migration
# ================================================================================
...
def test_migration_seeds_slugs_and_role_grants(seed_db):
    ...
    seed_migration.seed_share_permissions(live_apps, None)
...
def test_migration_reverse_removes_the_slugs(seed_db):
    seed_migration.seed_share_permissions(live_apps, None)
    seed_migration.remove_share_permissions(live_apps, None)
