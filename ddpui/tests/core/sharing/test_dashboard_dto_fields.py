...
fields the frontend's ShareModal follow-up needs — `general_audience`,
`general_level` (straight off the model columns) and `is_owner`/
`is_creator` (viewer-relative, computed from `owner_id`/`created_by_id`
against the caller's `orguser.id` — no extra query per row).
...
class TestDashboardListDTOFields:
    def test_owner_sees_is_owner_true(self, org, member, analyst):
        dashboard = _dashboard(org, member, GeneralAudience.ALL_USERS, GeneralLevel.EDIT)
        ...
        assert response.general_audience == GeneralAudience.ALL_USERS
        assert response.general_level == GeneralLevel.EDIT
    ...
    def test_service_layer_list_query_count_unaffected(
        ...
        """The new fields (general_audience/general_level/is_owner/
        is_creator) read straight off columns already on the fetched row
        and never touch `.owner`/`.created_by` FK objects, so they add no
        query of their own. The binding "no N+1" guarantee for
...
class TestDashboardDetailDTOFields:
    def test_detail_carries_same_fields(self, org, member, analyst):
        dashboard = _dashboard(org, member, GeneralAudience.PRIVATE, GeneralLevel.VIEW)
        request = mock_request(member)
        ...
        assert response.is_owner is True
        assert response.is_creator is True
        assert response.general_audience == GeneralAudience.PRIVATE
        assert response.general_level == GeneralLevel.VIEW
