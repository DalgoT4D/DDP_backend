...
def test_drop_trial_database(mock_settings, mock_psycopg2):
    ...
    warehouse_provision.drop_trial_database(tc.id)

    executed = " ".join(str(c.args[0]) for c in cursor.execute.call_args_list)
    assert "DROP DATABASE IF EXISTS" in executed
    assert f"trial_{tc.id}" in executed
    # connection closed and autocommit set (DROP DATABASE cannot run in a transaction)
    assert conn.autocommit is True
    conn.close.assert_called_once()
