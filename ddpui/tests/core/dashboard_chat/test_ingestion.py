"""Tests for dashboard chat dbt docs generation and ingestion pipeline."""
...
def test_generate_dashboard_chat_dbt_docs_artifacts_updates_hashes(org, orgdbt, tmp_path):
    """dbt docs generation should write profiles, load artifacts, and persist hashes."""
    ...
    assert orgdbt.ai_manifest_sha256 == artifacts.manifest_sha256
    assert orgdbt.ai_catalog_sha256 == artifacts.catalog_sha256
    assert orgdbt.ai_docs_generated_at is not None


def test_ingest_org_is_idempotent_and_removes_disabled_sources(org, orgdbt, orguser, dashboard):
    """A second identical ingest should skip writes, and disabled sources should be deleted."""
        ...
        manifest_json={
            ...
            "sources": {
                "source.dashchat.raw.program_reach": {
                    "unique_id": "source.dashchat.raw.program_reach",
                    "source_name": "raw",
                    "schema": "raw",
                    "name": "program_reach",
                    "columns": {
            ...
            "nodes": {
                "model.dashchat.fact_program_reach": {
                    "unique_id": "model.dashchat.fact_program_reach",
                    "resource_type": "model",
                    "package_name": "dashchat",
                    "schema": "analytics",
    ...
    preferences.save(update_fields=["ai_chat_source_config", "updated_at"])
    ...
    assert "dashboard_context" in second_result.skipped_source_types
    assert len(vector_store.upsert_calls) == upsert_count_after_first_ingest
    assert "dashboard_context" in third_result.disabled_source_types
...
def test_ingest_org_keeps_collections_isolated_per_org(org, orgdbt, orguser, dashboard, seed_db):
    """The ingestion pipeline should never mix documents between org collections."""
    ...
    OrgPreferences.objects.create(org=org, ai_org_context_markdown="Org one")
    OrgPreferences.objects.create(org=other_org, ai_org_context_markdown="Org two")
