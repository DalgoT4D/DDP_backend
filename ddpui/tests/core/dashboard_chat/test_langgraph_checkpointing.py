...
def _seed_ready_metadata_artifact(primary_dashboard: Dashboard) -> None:
    """Create the minimal ready metadata artifact required by runtime tests."""
    DashboardChatMetadataArtifact.objects.create(
        dashboard=primary_dashboard,
        status=DashboardChatMetadataArtifactStatus.READY,
        artifact_json=DashboardChatMetadataArtifactPayload(
...
def test_load_context_node_bootstraps_checkpoint_payloads(primary_dashboard):
    ...
    _seed_ready_metadata_artifact(primary_dashboard)
    ...
    assert updates["metadata_artifact_status"] == DashboardChatMetadataArtifactStatus.READY
    assert updates["metadata_artifact_payload"]["allowlisted_tables"] == [
...
def test_runtime_rebuilds_response_for_non_small_talk_turns_in_same_session(primary_dashboard):
    ...
    query_with_sql_updates = {
        ...
        "sql_validation": DashboardChatSqlValidationResult(
            is_valid=True,
