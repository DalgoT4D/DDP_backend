...
# LangGraph executes nodes on worker threads, so these tests need committed fixture
# data instead of pytest-django's default per-test transaction wrapper.
...
def primary_dashboard(org, orguser, primary_chart):
    dashboard = Dashboard.objects.create(
        ...
        last_modified_by=orguser,
        org=org,
        ai_context_markdown="## Reach metric\n\nTracks beneficiaries served per program.",
    )
    yield dashboard
    dashboard.delete()
...
def related_dashboard(org, orguser, related_chart):
    dashboard = Dashboard.objects.create(
        ...
        last_modified_by=orguser,
        org=org,
        ai_context_markdown="## Donor context\n\nTracks donor-wise funding.",
    )
    yield dashboard
    dashboard.delete()
...
@pytest.fixture
def preferences(org):
    pref = OrgPreferences.objects.create(
        org=org,
        ai_data_sharing_enabled=True,
        ai_org_context_markdown="## Organization context\n\nDalgo supports NGO dashboards.",
...
def test_runtime_data_query_uses_distinct_values_before_sql_execution(
    ...
    transaction.commit()
    chart_id = next(iter(primary_dashboard.components.values()))["config"]["chartId"]
    vector_store = FakeVectorStore(
        [
            DashboardChatVectorQueryResult(
                ...
                metadata={
                    "source_type": "dashboard_export",
                    "source_identifier": f"dashboard:{primary_dashboard.id}:chart:{chart_id}",
                    "dashboard_id": primary_dashboard.id,
                },
                distance=0.01,
            )
    ...
    response = runtime.run(
        ...
        conversation_history=[DashboardChatMessage(role="user", content="Show me beneficiary data")],
    )
