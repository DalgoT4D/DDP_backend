...
def test_prompt_store_returns_default_when_no_db_override_exists():
    ...
    assert '"How many beneficiaries participated?" means count beneficiaries, not sum services delivered.' in (
        DEFAULT_DASHBOARD_CHAT_PROMPTS[DashboardChatPromptTemplateKey.NEW_QUERY_SYSTEM]
    )
...
def test_default_new_query_prompt_requires_joins_for_cross_table_questions():
    ...
    assert "identify the exact entity, measure, grain, time scope, threshold, and every requested part or comparison" in prompt
    assert "match the counting method to the table grain" in prompt
    assert "entity_counting_guidance" in prompt
    assert "the SQL is incomplete unless every named entity or period appears in the result" in prompt
