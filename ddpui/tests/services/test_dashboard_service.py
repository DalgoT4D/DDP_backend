...
# ================================================================================
# Test compute_field_changes (Audit Log Support)
# ================================================================================


class TestComputeFieldChanges:
    """Tests for DashboardService.compute_field_changes()"""

    def test_no_changes_returns_empty_dict(self, orguser, org, seed_db):
        """When dashboards are identical, returns empty dict."""
        dashboard = Dashboard.objects.create(
            title="Test Dashboard",
            description="Test Description",
            dashboard_type="native",
            grid_columns=12,
            tabs=[{"id": "tab-1", "title": "Tab 1", "layout_config": [], "components": {}}],
            created_by=orguser,
            org=org,
        )

        result = DashboardService.compute_field_changes(dashboard, dashboard)

        assert result == {}

        dashboard.delete()

    def test_detects_basic_field_changes(self, orguser, org, seed_db):
        """Detects changes to basic fields: title and description."""
        ...
        result = DashboardService.compute_field_changes(old_dashboard, new_dashboard)

        assert result["title"] == {"old": "Old Title", "new": "New Title"}
        assert result["description"] == {"old": "Old Description", "new": "New Description"}

    def test_detects_tab_changes(self, orguser, org, seed_db):
        """Detects tab changes: add, remove, and rename."""
        # Test tab added
        old_dashboard = Dashboard(
            title="Title",
            dashboard_type="native",
        ...
        new_dashboard = Dashboard(
            ...
            tabs=[
                ...
                {"id": "tab-2", "title": "New Tab", "layout_config": [], "components": {}},
            ],
            created_by=orguser,
            org=org,
        )
        result = DashboardService.compute_field_changes(old_dashboard, new_dashboard)
        assert "Tab 'New Tab'" in result
        assert result["Tab 'New Tab'"]["new"] == "added"

        # Test tab removed
        result = DashboardService.compute_field_changes(new_dashboard, old_dashboard)
        assert "Tab 'New Tab'" in result
        assert result["Tab 'New Tab'"]["old"] == "removed"

        # Test tab renamed
        old_dashboard.tabs = [
            {"id": "tab-1", "title": "Old Tab Name", "layout_config": [], "components": {}}
        ]
        new_dashboard.tabs = [
            {"id": "tab-1", "title": "New Tab Name", "layout_config": [], "components": {}}
        ]
        result = DashboardService.compute_field_changes(old_dashboard, new_dashboard)
        assert "Tab renamed" in result
        assert "Old Tab Name" in result["Tab renamed"]["old"]
        assert "New Tab Name" in result["Tab renamed"]["new"]

    def test_detects_component_changes(self, orguser, org, seed_db):
        """Detects component changes: chart/KPI added and removed."""
        old_dashboard = Dashboard(
            title="Title",
            dashboard_type="native",
            tabs=[{"id": "tab-1", "title": "Tab 1", "layout_config": [], "components": {}}],
            ...
            tabs=[
                {
                    ...
                    "layout_config": [],
                    "components": {
                        "chart-1": {"type": "chart", "title": "Sales Chart", "config": {}},
                        "kpi-1": {"type": "kpi", "title": "Revenue KPI", "config": {}},
                    },
                }
            ],
            created_by=orguser,
        ...
        result = DashboardService.compute_field_changes(old_dashboard, new_dashboard)
        assert "Chart added" in result
        assert "Sales Chart" in result["Chart added"]["new"]
        assert "Kpi added" in result
        assert "Revenue KPI" in result["Kpi added"]["new"]

        # Test components removed
        result = DashboardService.compute_field_changes(new_dashboard, old_dashboard)
        assert "Chart removed" in result
        assert "Sales Chart" in result["Chart removed"]["old"]
