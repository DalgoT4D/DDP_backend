...
class TestComputeFieldChanges:
    """Tests for ChartService.compute_field_changes()"""

    def test_no_changes_returns_empty_dict(self, orguser, org, seed_db):
        """When charts are identical, returns empty dict."""
        chart = Chart.objects.create(
            title="Test Chart",
            description="Test Description",
            chart_type="bar",
            schema_name="public",
            table_name="users",
            extra_config={"metric": "count"},
            created_by=orguser,
            last_modified_by=orguser,
            org=org,
        )

        result = ChartService.compute_field_changes(chart, chart)
    ...
    def test_detects_basic_field_changes(self, orguser, org, seed_db):
        """Detects changes to basic fields: title, description, chart_type, schema_name, table_name."""
        old_chart = Chart(
            title="Old Title",
        ...
        result = ChartService.compute_field_changes(old_chart, new_chart)

        assert result["title"] == {"old": "Old Title", "new": "New Title"}
        assert result["description"] == {"old": "Old Description", "new": "New Description"}
        assert result["chart_type"] == {"old": "bar", "new": "line"}
        assert result["schema_name"] == {"old": "public", "new": "analytics"}
        assert result["table_name"] == {"old": "users", "new": "orders"}

    def test_detects_config_changes(self, orguser, org, seed_db):
        """Detects config changes: simple values, nested values, and list changes."""
        old_chart = Chart(
            title="Title",
            chart_type="bar",
            schema_name="public",
        ...
        new_chart = Chart(
            ...
            extra_config={
                "aggregation": "avg",
                "customizations": {"fontSize": 16},
                "metrics": [{"name": "count"}, {"name": "sum"}],
            },
            created_by=orguser,
            org=org,
        )
        ...
        # Simple value change
        assert result["config.aggregation"] == {"old": "sum", "new": "avg"}
        # Nested value change
        assert result["config.customizations.fontSize"] == {"old": 12, "new": 16}
        # List length change
        assert result["config.metrics"] == {"old": "1 items", "new": "2 items"}
