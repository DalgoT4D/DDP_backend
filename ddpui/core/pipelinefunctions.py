...
def _normalize_airbyte_column_name(name: str) -> str:
    """Airbyte Destinations V2 rule: chars not in [a-zA-Z0-9_$] → underscore, case preserved.

    Applied at connection-save time so the block's `extra.post_sync_ops` reflects the
    actual DB column names Airbyte writes (not the raw user-supplied names).
    """
...
def build_connection_block_extra(org_task: OrgTask) -> dict:
    ...
    Contains env (secret block ref) and post_sync_ops (raw config — SQL is generated by
    prefect-proxy at flow-run time to preserve live table partition/cluster specs).
    ...
    if org_task.post_sync_transform:
        ...
        if not warehouse:
            ...
            for op in org_task.post_sync_transform.get("ops", []):
                if op.get("type") != "cast":
                    continue
                post_sync_ops.append(
                    {
                        "type": "cast",
                        "schema": op["schema"],
                        "table": op["table"],
                        "column_casts": {
                            _normalize_airbyte_column_name(col): cast_type
                            for col, cast_type in op["config"].items()
                        },
                    }
                )
