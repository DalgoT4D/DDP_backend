...
def route_intent_node(state: DashboardChatGraphState, llm_client) -> dict[str, Any]:
    ...
    if fast_path_intent is not None:
        return {
            "conversation_context": serialize_conversation_context(conversation_context),
            "intent_decision": serialize_intent_decision(fast_path_intent),
    ...
    return {
        "conversation_context": serialize_conversation_context(conversation_context),
        "intent_decision": serialize_intent_decision(intent_decision),
    }
