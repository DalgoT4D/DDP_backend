"""Ingestion pipeline for dashboard chat retrieval context."""
...
TOGGLEABLE_SOURCE_TYPES = {
    DashboardChatSourceType.ORG_CONTEXT.value: "org_context",
    DashboardChatSourceType.DASHBOARD_CONTEXT.value: "dashboard_context",
    DashboardChatSourceType.DBT_MANIFEST.value: "dbt_manifest",
    DashboardChatSourceType.DBT_CATALOG.value: "dbt_catalog",
}
INGEST_SOURCE_ORDER = [
...
class DashboardChatIngestionError(Exception):
    """Raised when the dashboard chat ingestion pipeline cannot complete."""


@dataclass(frozen=True)
class DashboardChatIngestionResult:
    """Summary of a completed org ingest run."""
    ...
    skipped_source_types: list[str]
    disabled_source_types: list[str]


def _normalize_text(value: str) -> str:
    """Normalize text before chunking so IDs stay deterministic."""
    return "\n".join(
...
def chunk_dashboard_chat_text(text: str, max_chars: int = MARKDOWN_CHUNK_MAX_CHARS) -> list[str]:
    """Chunk text deterministically by blank-line blocks and then hard-wrap if needed."""
...
class DashboardChatIngestionService:
    ...
    def ingest_org(self, org: Org) -> DashboardChatIngestionResult:
        """Generate and ingest all enabled dashboard chat sources for an org."""
        if org.dbt is None:
            raise DashboardChatIngestionError(
                "dbt workspace not configured for dashboard chat ingest"
            )

        preferences = OrgPreferences.objects.filter(org=org).first()
        source_config = self._get_source_config(preferences)
        dbt_docs = None
        if source_config["dbt_manifest"] or source_config["dbt_catalog"]:
            dbt_docs = self.dbt_docs_generator(org, org.dbt)
        desired_documents = self._build_documents(org, preferences, dbt_docs, source_config)
        ...
        for source_type in INGEST_SOURCE_ORDER:
            ...
            toggle_key = TOGGLEABLE_SOURCE_TYPES.get(source_type_value)
            if toggle_key and not source_config[toggle_key]:
                existing_documents = self.vector_store.get_documents(
                    org.id,
                ...
                if existing_documents:
                    self.vector_store.delete_documents(
                        org.id,
                        ids=[document.document_id for document in existing_documents],
                    )
                    deleted_document_ids.extend(
                        document.document_id for document in existing_documents
                    )
                disabled_source_types.append(source_type_value)
                continue

            source_documents = desired_documents[source_type_value]
            existing_documents = self.vector_store.get_documents(
                org.id, source_types=[source_type_value]
            )
            existing_document_ids = {document.document_id for document in existing_documents}
            desired_document_ids = {document.document_id for document in source_documents}
            ...
            if stale_document_ids:
                ...
                deleted_document_ids.extend(stale_document_ids)
            ...
            if new_documents:
                upserted_document_ids.extend(
                    self.vector_store.upsert_documents(org.id, new_documents)
                )
            elif not stale_document_ids:
                skipped_source_types.append(source_type_value)

        vector_ingested_at = timezone.now()
        org.dbt.ai_vector_last_ingested_at = vector_ingested_at
        org.dbt.save(update_fields=["ai_vector_last_ingested_at", "updated_at"])

        return DashboardChatIngestionResult(
            org_id=org.id,
            docs_generated_at=(
                dbt_docs.generated_at if dbt_docs is not None else org.dbt.ai_docs_generated_at
            ),
            vector_ingested_at=vector_ingested_at,
            source_document_counts={
                source_type.value: len(desired_documents[source_type.value])
                for source_type in INGEST_SOURCE_ORDER
            },
            upserted_document_ids=sorted(upserted_document_ids),
            deleted_document_ids=sorted(deleted_document_ids),
            skipped_source_types=sorted(skipped_source_types),
            disabled_source_types=sorted(disabled_source_types),
        )

    @staticmethod
    def _get_source_config(preferences: OrgPreferences | None) -> dict[str, bool]:
        """Return a normalized source toggle dict for an org."""
        source_config = default_ai_chat_source_config()
        if preferences is None or preferences.ai_chat_source_config is None:
            return source_config
        source_config.update(
            {
                key: bool(preferences.ai_chat_source_config.get(key, default))
                for key, default in source_config.items()
            }
        )
        return source_config
    ...
    def _build_documents(
        ...
        if source_config["org_context"] and preferences and preferences.ai_org_context_markdown:
            documents_by_source[DashboardChatSourceType.ORG_CONTEXT.value].extend(
                self._build_markdown_documents(
                    ...
                    markdown=preferences.ai_org_context_markdown,
                    updated_at=preferences.ai_org_context_updated_at,
                )
            )

        dashboards = list(Dashboard.objects.filter(org=org).order_by("id"))
        if source_config["dashboard_context"]:
            for dashboard in dashboards:
                if not dashboard.ai_context_markdown:
                    continue
                documents_by_source[DashboardChatSourceType.DASHBOARD_CONTEXT.value].extend(
                    self._build_markdown_documents(
                        ...
                        markdown=dashboard.ai_context_markdown,
                        dashboard_id=dashboard.id,
                        updated_at=dashboard.ai_context_updated_at,
                    )
                )
        ...
        if source_config["dbt_manifest"]:
            if dbt_docs is None:
                raise DashboardChatIngestionError("dbt manifest ingest requested without dbt docs")
            documents_by_source[DashboardChatSourceType.DBT_MANIFEST.value].extend(
                self._build_manifest_documents(org.id, dbt_docs)
            )

        if source_config["dbt_catalog"]:
            if dbt_docs is None:
                raise DashboardChatIngestionError("dbt catalog ingest requested without dbt docs")
    ...
    def _build_dashboard_export_documents(
        ...
        summary_lines = [
            f"Dashboard title: {dashboard_payload.get('title')}",
    ...
    def _build_catalog_documents(
        ...
        manifest_project_name = dbt_docs.manifest_json.get("metadata", {}).get("project_name")
        documents: list[DashboardChatVectorDocument] = []

        for unique_id, source in sorted((catalog_json.get("sources") or {}).items()):
            if not self._include_dbt_unique_id(unique_id, manifest_project_name):
                continue
            documents.extend(
                self._build_markdown_documents(
                    org_id=org_id,
    ...
    def _include_dbt_unique_id(unique_id: str, project_name: str | None) -> bool:
        ...
        if package_name in DBT_IGNORE_PACKAGES:
            return False
        if project_name and parts[0] == "model" and package_name != project_name:
            return False
        return True
