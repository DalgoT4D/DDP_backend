...
class DashboardChatAllowlist:
    ...
    chart_id_to_table: dict[int, str] = field(default_factory=dict)
    ...
    def prioritized_tables(
        self,
        selected_chart_id: int | None = None,
        limit: int | None = None,
    ) -> list[str]:
        """Return direct chart tables first, then lineage tables."""
        ordered_tables: list[str] = []

        if selected_chart_id is not None and selected_chart_id in self.chart_id_to_table:
            ordered_tables.append(self.chart_id_to_table[selected_chart_id])

        ordered_tables.extend(sorted(self.chart_tables - set(ordered_tables)))
        ordered_tables.extend(
            sorted(self.upstream_tables - set(ordered_tables) - self.chart_tables)
        )
    ...
    def build(
        ...
        for chart in export_payload.get("charts") or []:
            ...
            if chart.get("id") is not None:
                allowlist.chart_id_to_table[int(chart["id"])] = table_name
