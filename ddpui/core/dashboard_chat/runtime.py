...
class DashboardChatRuntimeState(TypedDict, total=False):
    ...
    org: Org
    dashboard_id: int
    selected_chart_id: int | None
    user_query: str
    conversation_history: list[DashboardChatMessage]
    preferences: OrgPreferences
    source_config: dict[str, bool]
    dashboard_export: dict[str, Any]
    dashboard_summary: str
    allowlist: DashboardChatAllowlist
    retrieval_source_types: dict[str, list[str]]
...
class DashboardChatRuntime:
    ...
    def run(
        ...
        conversation_history: Sequence[DashboardChatMessage | dict[str, str]] | None = None,
    ) -> DashboardChatResponse:
    ...
    def _build_graph(self):
        ...
        graph.add_node("initialize_context", self._node_initialize_context)
        graph.add_node("route_intent", self._node_route_intent)
        graph.add_node("load_retrieval_context", self._node_load_retrieval_context)
        graph.add_node("retrieve_docs", self._node_retrieve_docs)
        graph.add_node("inspect_schema_context", self._node_inspect_schema_context)
        graph.add_node("plan_query", self._node_plan_query)
        graph.add_node("distinct_value_lookup", self._node_distinct_value_lookup)
        ...
        graph.add_edge("initialize_context", "route_intent")
        graph.add_conditional_edges(
            "route_intent",
            self._route_after_intent,
            {
                "compose_answer": "compose_answer",
                "load_retrieval_context": "load_retrieval_context",
            },
        )
        graph.add_edge("load_retrieval_context", "retrieve_docs")
        graph.add_edge("retrieve_docs", "inspect_schema_context")
        graph.add_edge("inspect_schema_context", "plan_query")
        graph.add_conditional_edges(
            "plan_query",
            self._route_after_plan,
            {
                "compose_answer": "compose_answer",
                "distinct_value_lookup": "distinct_value_lookup",
            },
        )
        graph.add_edge("distinct_value_lookup", "generate_sql")
        ...
        graph.add_edge("compose_answer", "finalize_response")
        graph.add_edge("finalize_response", END)
        return graph.compile()

    def _node_initialize_context(
        self,
        state: DashboardChatRuntimeState,
    ) -> DashboardChatRuntimeState:
        """Load dashboard context, toggles, and lineage allowlist."""
        org = state["org"]
        preferences = AiDashboardChatSettingsService.get_or_create_preferences(org)
        source_config = AiDashboardChatSettingsService.normalize_source_config(
            preferences.ai_chat_source_config
        )
        dashboard_export = DashboardService.export_dashboard_context(state["dashboard_id"], org)
        manifest_json = (
            DashboardChatAllowlistBuilder.load_manifest_json(org.dbt)
            if org.dbt_id and source_config.get("dbt_manifest")
            else None
        )
        ...
        state["dashboard_summary"] = self._build_dashboard_summary(
            dashboard_export=dashboard_export,
            selected_chart_id=state.get("selected_chart_id"),
        )
    ...
    def _node_load_retrieval_context(
        self,
        state: DashboardChatRuntimeState,
    ) -> DashboardChatRuntimeState:
        """Map org source toggles into retrieval source groups."""
        source_config = state["source_config"]
        dashboard_source_types = [DashboardChatSourceType.DASHBOARD_EXPORT.value]
        if source_config.get("dashboard_context"):
            dashboard_source_types.append(DashboardChatSourceType.DASHBOARD_CONTEXT.value)

        org_source_types = []
        if source_config.get("org_context"):
            org_source_types.append(DashboardChatSourceType.ORG_CONTEXT.value)

        dbt_source_types = []
        if source_config.get("dbt_manifest"):
            dbt_source_types.append(DashboardChatSourceType.DBT_MANIFEST.value)
        if source_config.get("dbt_catalog"):
            dbt_source_types.append(DashboardChatSourceType.DBT_CATALOG.value)

        state["retrieval_source_types"] = {
            "dashboard": dashboard_source_types,
            "org": org_source_types,
            "dbt": dbt_source_types,
        }
        return state

    def _node_retrieve_docs(
        self,
        state: DashboardChatRuntimeState,
    ) -> DashboardChatRuntimeState:
        """Retrieve current-dashboard, org, and dbt context from Chroma."""
        org = state["org"]
        retrieval_source_types = state["retrieval_source_types"]
        dashboard_results = self._query_vector_store(
            org=org,
            query_text=state["user_query"],
            source_types=retrieval_source_types["dashboard"],
            dashboard_id=state["dashboard_id"],
        )
        org_results = self._query_vector_store(
            org=org,
            query_text=state["user_query"],
            source_types=retrieval_source_types["org"],
        )
        dbt_results = self._filter_allowlisted_dbt_results(
            self._query_vector_store(
                org=org,
                query_text=state["user_query"],
                source_types=retrieval_source_types["dbt"],
            ),
            state["allowlist"],
        )
    ...
    def _node_inspect_schema_context(
        self,
        state: DashboardChatRuntimeState,
    ) -> DashboardChatRuntimeState:
    ...
    def _node_distinct_value_lookup(
        self,
        state: DashboardChatRuntimeState,
    ) -> DashboardChatRuntimeState:
    ...
    def _normalize_conversation_history(
        conversation_history: Sequence[DashboardChatMessage | dict[str, str]] | None,
    ) -> list[DashboardChatMessage]:
        """Normalize conversation history into typed messages."""
        normalized_messages: list[DashboardChatMessage] = []
        for item in conversation_history or []:
            if isinstance(item, DashboardChatMessage):
    ...
    def _heuristic_intent_decision(
        user_query: str,
        conversation_history: Sequence[DashboardChatMessage],
    ...
    def _merge_retrieval_results(
        ...
        for priority, result_group in enumerate([dashboard_results, org_results, dbt_results]):
            for result in result_group:
                ...
                if selected_chart_id is not None and f":chart:{selected_chart_id}" in result.source_identifier:
                    effective_priority = -1
                scored_results.append(
                    (
                        (effective_priority, result.distance if result.distance is not None else 999.0),
                        result,
                    )
                )
    ...
    def _build_dashboard_summary(
        ...
        for chart in dashboard_export.get("charts") or []:
            chart_label = f"Chart {chart.get('id')}: {chart.get('title')}"
            if selected_chart_id is not None and chart.get("id") == selected_chart_id:
                chart_label += " [selected]"
            lines.append(
                f"{chart_label} uses {chart.get('schema_name')}.{chart.get('table_name')} ({chart.get('chart_type')})"
            )
        return "\n".join(lines)
